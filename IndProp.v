...
Proof.
  intros n H. inversion H as [|n' H1 H2]. inversion H1. apply H3.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. induction H.
    + apply even'_0.
    + apply (even'_sum 2 n even'_2 IHeven).
Qed.
(** [] *)
...
Proof.
  ...
  - apply IHeven. simpl in H1. apply evSS_ev in H1. apply H1.
Qed.
(** [] *)
...
Proof.
  intros n m p H1 H2. apply (ev_ev__ev (n+p)).
  - rewrite (plus_comm n p). rewrite (plus_comm m p). rewrite plus_swap. rewrite plus_assoc.  
    rewrite plus_assoc.  rewrite <- double_plus. rewrite <- (plus_assoc (double p) n m). apply ev_sum.
    + apply ev_double.
    + apply H1.
  - apply H2.
Qed.
...
Proof.
  ...
  - assert (I: S n1 <= S (n1 + n2)). { apply n_le_m__Sn_le_Sm. apply le_plus_l. }
    apply (le_trans (S n1) (S (n1 + n2)) m I) in H. apply H.
  - assert (I: S n2 <= S (n1 + n2)). { apply n_le_m__Sn_le_Sm. 
    rewrite -> plus_comm. apply le_plus_l. }
   apply (le_trans (S n2) (S (n1 + n2)) m I) in H. apply H.
Qed.
...
Proof.
  intros.apply c2. apply c3. apply c1.
Qed.
*)
*)
...
Proof.
  ...
  - intros. unfold fR. induction H.
    ...
    + simpl in IHR. apply S_injective in IHR. rewrite <- plus_n_Sm in IHR.
      apply S_injective in IHR. apply IHR.
    + rewrite -> plus_comm. apply IHR.
  - unfold fR. intros. destruct H.
    + induction m.
      * induction n. simpl. apply c1. simpl. apply c3. simpl in IHn. apply IHn.
...
Proof.
  intros. induction l.
  - apply nil_is_subseq.
  - apply combine_subseq. apply IHl.
Qed.
...
Proof.
  ...
  - intros. inversion H. apply nil_is_subseq.
  - intros. inversion H.
    + apply nil_is_subseq.
    + apply combine_subseq. apply IHsubseq. apply H3.
    + apply subseq_larger. apply IHsubseq. apply H3.
  - intros. apply subseq_larger. apply IHsubseq. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. apply MStarApp.
    ...
    + simpl. apply IHss. intros. apply H. simpl. right. apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  - inversion Heqre'. exists []. split.
    + reflexivity.
    + intros. inversion H.
  - inversion Heqre'. rewrite -> H2 in *. induction s1.
    + intros. simpl. apply IHexp_match2. apply Heqre'.
    + intros. apply IHexp_match2 in Heqre'. destruct Heqre'. destruct H1.
...
Proof.
  ...
  - inversion H. unfold not in H0. split.
    + intros. exfalso. apply H0. apply H1.
    + intros. inversion H1. 
Qed.
(** [] *)
...
Theorem eqbP_practice : forall n l,
  count n l = 0 -> ~(In n l).
Proof.
  unfold not. intros n l. induction l as [|h t IH].
  - intros. inversion H0.
  - simpl. destruct (eqbP n h).
    ...
    + simpl. intros. apply IH.
      * apply H0.
      * destruct H1.
        { exfalso. apply H. symmetry in H1. apply H1. }
        { apply H1. }
Qed.
(** [] *)
...
Proof.
  unfold not. intros. inversion H. inversion H4. assert(H': 1 = 1). 
  { reflexivity. } apply H7 in H'. destruct H'.
Qed.
(* 
  Proof. intro.
...
Proof.
  intros. generalize dependent x. induction l as [| h t IH].
  - intros. inversion H.
  - intros. simpl in H. destruct H.
    + rewrite -> H. exists []. exists t. simpl. reflexivity.
    + apply IH in H. destruct H. destruct H. exists (h :: x0). exists x1. simpl.
...
Proof.
  ...
  - intros. inversion H. destruct s1 as [| s1_h s1_tl].
    + left. simpl. split.
      * apply H3. * apply H4.
    + simpl in H1. inversion H1. right. exists s1_tl, s2. split.
      * reflexivity.
      * split.
        { rewrite <- H6. apply H3. }
        { apply H4. }
  - intros. destruct H.
    + destruct H as [H1 H2]. apply (MApp [] re0 (a::s) re1 H1 H2).
    + destruct H as [s0 [s1]]. destruct H as [H1 [H2 H3]]. rewrite H1.
      apply (MApp (a::s0) re0 s1 re1 H2) in H3. simpl in H3. apply H3.
Qed.
(** [] *)
  ...
  - intros. remember (a :: s) as l. remember (Star re) as re'.
    ...
    + destruct s1' as [| s1'_h s1'_tl].
      *
        ...
        apply Heqre'.
      * simpl in Heql. inversion Heql. exists s1'_tl, s2'. split.
        { reflexivity. }
        { split.
          - rewrite H0 in Hmatch1. inversion Heqre'. rewrite <- H2.
            apply Hmatch1.
          - apply Hmatch2. }
  - intros. destruct H as [s0 [s1]]. destruct H as [H1 [H2 H3]].
