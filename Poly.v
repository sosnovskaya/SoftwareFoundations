...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?  (Add YES or NO to each line.)
      - [d (b a 5)] NO Check - error
      - [d mumble (b a 5)] YES
      - [d bool (b a 5)]  YES
      - [e bool true] YES
      - [e mumble (b c 0)] YES
      - [e bool (b c 0)]  NO Check - error
      - [c]  NO *)

Check d mumble (b a 5). (* d mumble (b a 5): grumble mumble*)
Check d bool (b a 5). (* d bool (b a 5): grumble bool*)
Check e bool true. (* e bool true : grumble bool *)
Check e mumble (b c 0). (* e mumble (b c 0) : grumble mumble *)
Check c. (*c : mumble*)
...
@combine : forall X Y : Type, list X -> list Y -> list (X * Y)
    ...
    Command result = [(1, false); (2, false)]
     : list (nat * bool)
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IH. rewrite -> map_assoc. reflexivity.
Qed.
(** [] *)
...
Definition succ (n : cnat) : cnat := fun X f x => n X f (f x).
...
(** Multiplication: *)
Definition mult (n m : cnat) : cnat := fun X f x => n X (m X f) x. 
...
Definition exp (n m : cnat) : cnat := fun X => m (X -> X) (n X). 
