...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P HP Q HQ. destruct HP. apply HQ.
Qed.
(** [] *)
...
(* Proof: We introduce proposition P and hypotesis H, which is P.
We need to froof ~~P as a result. So we unfold operarot not to 
(P -> False) -> False and introduce new hypotesis G : P -> False. Now we 
apply G and get False.In a couple of words P holds/leads False, so we need 
just to proove P to prove the Theorem. And we apply H.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H1 H2. unfold not in H2. unfold not. intros P'. apply H2.
  apply H1. apply P'.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros H. destruct H as [H1 H2]. apply H2. apply H1.
Qed.
(** [] *)
...
(* 
  We introduce proposition P and unfold operator not. Then we introduce 
hypotesis H:  P /\ (P -> False). We destruct this conjunction on left and 
right.And apply them one by one.
...
Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R [H1L H1R] [H2L H2R]. split.
  - intros P'. apply H2L. apply H1L. apply P'.
  - intros R'. apply H1R. apply H2R. apply R'.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. split.
    + destruct H.
      { left. apply H. }
      { right. destruct H as [H1 H2]. apply H1. }
    + destruct H.
      { left. apply H. }
      { right. destruct H as [H1 H2]. apply H2. }
  - intros H . destruct H as [H1 H2].  destruct H1.
    + left. apply H.
    + destruct H2.
      { left. apply H0.  }
      { right. split.
...
Proof.
  intros X P H1 H2. unfold not in H2. destruct H2. apply H. apply H1.
Qed.
(** [] *)
...
Proof.
  intros X P Q. split.
  - intros H. destruct H as [X' H]. destruct H as [H1|H2].
...
Proof.
  ...
  - induction l as [|h l' IH].
    + simpl. intros H1. contradiction.
    + intros H. simpl in H. destruct H.
      * exists h. split. apply H. simpl. left. reflexivity.
      * apply IH in H. destruct H. destruct H. exists x. split.
        { apply H. }
...
Proof.
  ...
  - induction l as [|h t IH].
    ...
    + simpl. intros H. apply or_assoc. destruct H.
      * left. apply H.
      * right. apply IH. apply H.
  - induction l as [|h t IH].
    + simpl. intros H. destruct H. destruct H. apply H.
    + simpl. intros H. destruct H. destruct H. left. apply H. right. 
      apply IH. left. apply H. right. apply IH. right. apply H.
Qed.
(** [] *)
...
Proof.
  intros X l1 l2. generalize dependent l2. induction l1 as [|h l1' IH].
  - intros. reflexivity.
  - intros. simpl. rewrite -> IH. rewrite <- app_assoc. simpl. reflexivity.
Qed.

Lemma tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof.
  intros X. apply functional_extensionality. intros x. unfold tr_rev.
  rewrite -> rev_append_correct. apply app_nil_r.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite -> evenb_S. destruct (evenb n).
    ...
    + simpl. destruct IHn. exists (S x). rewrite -> H. reflexivity.
Qed.
(** [] *)
...
intros.
  split.
  - unfold not. intros. rewrite H0 in H. simpl in H. 
    rewrite -> (eqb_neq_helper y) in H. discriminate.
  - unfold not. generalize dependent y. induction x as [|x'].
    + simpl. destruct y.
      * intros. assert(H': 0=0). { reflexivity. } apply H in H'. destruct H'.
      * intros. reflexivity.
    + intros. destruct y.
      * simpl. reflexivity.
      * simpl. apply (IHx' y). intros. assert(H': S x' = S y).
        { rewrite H0. reflexivity. } apply H in H'. destruct H'.
Qed.
(** [] *)
...
Proof.
  ...
  - generalize dependent l2. induction l1 as [| x l1' IHl'].
    ...
    + intros. simpl in H0. destruct l2.
      * discriminate.
      * apply andb_true_iff in H0.  destruct H0. apply H in H0.
        apply IHl' in H1. rewrite H0. rewrite H1. reflexivity.
  - generalize dependent l2. induction l1 as [| x l1' IHl1'].
    ...
    + intros. simpl. destruct l2.
      * discriminate.
      * injection H0. intros. apply (H x x0) in H2.  apply IHl1' in H1.
...
Proof.
  ...
  - induction l as [| x' l' IHl'].
    ...
    + simpl. intros. apply andb_true_iff in H. destruct H. split.
      * apply H.
      * apply IHl' in H0. apply H0.
  - induction l as [| x' l' IHl'].
    + intros. simpl. reflexivity.
    + simpl. intros. destruct H. rewrite H. apply IHl' in H0.
...
Proof.
  unfold excluded_middle. unfold not. intros. destruct (H (P x)).
  - apply H1.
