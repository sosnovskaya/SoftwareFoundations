...
Theorem rev_exercise1 : forall (l l' : list nat),
     ...
     l' = rev l.
Proof.
  intros l l' q1. symmetry. rewrite -> q1. apply rev_involutive.
Qed.
(** [] *)
...
(* Rewrite matches a term with an equation, and replaces the term
   ...
   Apply will do this too but it has extra functionality  In some way it 
   replace the term and execute it if it's possible.
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros X x y z l j H1 H2. injection H1 as H1h1 H1h2. injection H2 as H2h1 H2h2.
  symmetry. apply H2h1.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. destruct m.
    + simpl in H. discriminate H.
    + simpl in H. injection H as H'. rewrite <- plus_n_Sm in H'. symmetry in H'.
       rewrite <- plus_n_Sm in H'. symmetry in H'. injection H' as H''. apply HI in H''.
...
Proof.
  ...
  - intros m eq. destruct m.
    + simpl in eq. discriminate eq.
    + simpl in eq. apply IH in eq. apply f_equal. apply eq.
Qed.
...
(* 
        Introduce n and proceed by induction on it. 
      - Case: [n = 0]. 
        We must show that forall m  0 =? m = true -> 0 = m. 
        Introduce some m and an equation.Destrcut m.
          +  Case: [m = 0]. 
           We must show that  0 =? 0 = true -> 0 = 0. 
           It's obvious.
          +  Case: [m = S m']. 
            We must show that  0 =? S m' = true -> 0 = S m'
            Simplificate equation. It discriminates.
       -  Case: [n = S n']. 
          We must show that forall m  S n' =? m = true -> S n' = m
          with the induction hypothesis that  n' =? m = true -> n' = m .
          Introduce some m and an equation.Destrcut m.
            +  Case: [m = 0]. 
            We must show that  S' n =? 0 = true -> S n' = 0. 
            Simplificate equation. It discriminates.
            +  Case: [m = S m']. 
            We must show that  S n' =? S m' = true -> S n' = S m'
            Simplificate equation. Apply induction hypothesis to equation. 
            Then apply f_equal for the main part and apply equation.  
 *)
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
     ...
     nth_error l n = None.
Proof.
  intros n X l H. generalize dependent n. induction l as [| h l' IH].
  - intros n H1. rewrite <- H1. reflexivity.
  - intros n H2. rewrite <- H2. simpl. apply IH. reflexivity.
Qed.
(*intros n X l. generalize dependent n. induction l as [|h l' IHl].
...
Proof.
  ...
  - intros l1 l2 H. simpl in H. destruct h. destruct (split l'). 
    injection H. intros H1 H2. rewrite <- H1. rewrite <-H2. simpl. assert (Hm: (x0, y0) = (x0, y0)).
    {reflexivity. } apply IHl' in Hm. rewrite -> Hm. reflexivity.
Qed.
(** [] *)
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f b. destruct (f b) eqn:Heqf.
  - destruct b.
    + rewrite -> Heqf. apply Heqf.
    + destruct (f true) eqn: Heqtrue.
      {apply Heqtrue. }
      {apply Heqf. }
  - destruct b.
    + destruct (f false) eqn: Heqfalse.
      {apply Heqf. }
      {apply Heqfalse. }
    + rewrite -> Heqf. apply Heqf.
Qed.
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)  
   ...
   Proof: *)
   (* 
    Let n be a nat. We prove by induction on n that, for any n, m (n =? m) = (m =? n).
    First, suppose n = 0. We must show that (0 =? m) = (m =? 0).
    Since n = 0,we destrcut m on 0 and S m', by the definition of eqb and nat we'd done this part.
    Second, suppose n = S n' . We must show that (S n =? m) = (m =? S n), with the induction hypothesis n =? m) = (m =? n) .
    We destrcut m on 0 and S m'. First case is like above ones and the second one is (S n =? S m) = (S m =? S n).
    Which is (n =? m) = (m =? n), if we simplify equation and we apply induction hypothesis.
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p H1 H2. apply eqb_true in H1. apply eqb_true in H2.
  rewrite -> H1. rewrite <- H2. symmetry. apply eqb_refl.
Qed.
(** [] *)
...
Lemma split_combine_helper : forall (X:Type) (l: list X),
    length l = 0 -> l = [].
Proof.
  intros X l H. destruct l as [|h l'].
  - reflexivity.
...
Proof.
  unfold split_combine_statement. intros X Y l1. induction l1 as [|h1 l1' IHl1].
  - intros l2 H. simpl in H. symmetry in H. apply split_combine_helper in H.
    rewrite -> H. simpl. reflexivity.
  - intros l2 H. simpl. destruct l2 as [|h2 l2'].
    + simpl in H. discriminate.
    + simpl in H. injection H. intros H'. apply IHl1 in H'.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros X test x l. generalize dependent x. induction l as [|h l' IHl].
  - intros x lf H. simpl in H. discriminate.
  - intros x lf H. simpl in H. destruct (test h) eqn:Heqtest.
    + injection H. intros H1 H2. rewrite -> H2 in Heqtest. apply Heqtest.
...
Proof.
  unfold existsb'. intros X test l. induction l as [|h l' IHl].
  - reflexivity.
